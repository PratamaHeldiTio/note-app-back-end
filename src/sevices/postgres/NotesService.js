...
class NoteService {
  ...
  async addNote({
    ...
    // membuat query object untuk postgres
    const query = {
      text: 'INSERT INTO notes VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING id',
      values: [id, title, body, tags, createdAt, updatedAt, owner],
    };

    // menjalankan query sekaligus mengambil nilai kembalian
  ...
  async getNotes(owner) {
    try {
      ...
      const result = await this._pool.query(query);
      const mapResultNotes = result.rows.map(mapDBToModel);

      await this._cacheService.set(`notes:${owner}`, JSON.stringify(mapResultNotes));
      return mapResultNotes;
    }
  }
